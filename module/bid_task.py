...
class State:
    ...
    def __init__(self, setting, state_idx="test") -> None:
        self.state = setting["complete"]
        self.end_rule = setting["end_rule"]  # 翻页结束标志
        self.list_url = setting["url"]
    ...
    def init_end_rule(self):
        ...
        logger.info(f"{self.state_idx}.complete is: {self.state}\n" + \
    ...
    def save_newest_and_interrupt(self, bid: Bid):
        """ 保存最新的
        """
        deep_set(self.settings, "newest", _bid_to_dict(bid))
...
class InterruptState(State):
    ...
    def __init__(self, setting, state_idx="test") -> None:
        super().__init__(setting, state_idx)
...
class BidTask:
    ...
    state: State  # init at _get_state_from_task
    bid: Bid
    bid_tag: BidTag
    web_brows: WebBrows
    bid_web: BidHtml
    ...
    def __init__(self, settings, task_name="test") -> None:
        ...
        # 取一个任务, 若queue 为空报错退出
        assert self._get_task(deep_get(settings, "task.queue"))
        self._get_state_from_task(self.task, self.state_idx)  # 获得当前state
    ...
    def _get_state_from_task(self, task: dict, state_idx):
        """ 判断当前task, 调用相应的state对象
        """
        self.state = InterruptState(task, state_idx) \
            if task["complete"] == "interrupt" else State(task, state_idx)

    def init_brows(self, setting):
        """ 初始化网页对象模型
        """
        self.bid_tag = BidTag(setting)
        self.bid = Bid(setting)
        self.web_brows = WebBrows(setting)
        self.bid_web = BidHtml(setting)
    ...
    def process_tag_list(self):
        ...
        for idx, tag in enumerate(self.tag_list):
            ...
            if not self.state.start:
                self.state.bid_is_start(self.bid.message)
                if not self.state.start:
                    continue

            self.process_bid(self.bid)
        logger.info(f"get {idx+1} tags")
...
if __name__ == "__main__":
    json_file = r"bid_settings\bid_settings_t.json"
    settings = read_json(json_file)
    task = BidTask(settings["zzlh"])
