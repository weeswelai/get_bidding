...
if __name__ == "__main__":
    ...
    ReadFromStr = 1  # 测试字符串 存进前缀树
    LoadFromFile = 0  # 将文本文件的内容存进前缀树中
    LoadFromFile_b = 0  # 从二进制文件读取前缀树
    SaveLocalB = 0  # 保存前缀树到二进制文件中
    Test_search_all = 0  # 测试检索整个字符串,返回符合的匹配项
    ...
    # 从二进制文件读取前缀树
    if LoadFromFile_b:
        title_trie.init_from_file_b()

    # 保存前缀树到二进制文件中
    if SaveLocalB:
        title_trie.save_local()

    # 测试检索整个字符串,返回符合的匹配项
    if Test_search_all:
        Test_search_all_fromFile = 0
        Test_search_all_fromStr = 0
        Test_search_all_fromStrLines = 1
        # 多行字符串转list
        if Test_search_all_fromStrLines:
            l_t = str_t.split("\n")
            list_t = []
            for line in l_t:
                list_t.append(eval(line))
            # search_all
            for line in list_t:
                result = title_trie.search_all(line[0])
                if result:
                    logger.info(f"get {result} from {line[0]}")
